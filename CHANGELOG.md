...
- **Live-Indexer löscht jetzt aus allen aktiven Locale-Indexen.** Die Einzel-Delete-Pfade (unveröffentlicht, noindex, hidden, Berechtigung) nutzten `tl_page.language ?: 'de'` — `language` steht aber nur auf Root-Pages, d.h. Unterseiten im `en`-Index wurden nie entfernt (FFA: `page-6`/`page-7` unter einer unveröffentlichten Root blieben im en-Index). Neu: `LivePageIndexer::deletePageEverywhere()`.
...
- **Relevanz-Sortierung: bei gleicher Relevanz jetzt das neueste Dokument zuerst.** Ohne expliziten Tie-Breaker ordnet Meilisearch gleich gut passende Treffer nach interner Doc-ID (= Indexier-Reihenfolge, Files kommen `ORDER BY path`) — „Geschäftsbericht" zeigte 2001, 2002, 2006 … oben. `DocumentIndexer::RANKING_RULES` endet jetzt auf `published_at:desc` (nach `exactness`, damit das Datum nie echte Treffer-Qualität überstimmt). PHP-seitige Re-Rankings (Multi-Locale-Merge, Type-Sort-Tie-Break) nutzen denselben Vergleich (`SearchService::compareRelevance`).
- **Relevanz: nahezu gleich relevante Treffer werden nach Aktualität sortiert** (`SearchService::rankByRelevanceWithRecency`, ε = 0.02 auf Meilisearchs Ranking-Score). Live-Befund nach dem Tie-Break: Geschäftsberichte 2001–2007 standen mit 0.997 weiter vor 2016–2024 (0.987) — nur weil der PDF-Text der alten Berichte mit dem Wort „Geschäftsbericht" beginnt (`exactness: matchesStart`). Treffer werden nach Score sortiert und in Gruppen geschnitten (Score ≥ Gruppen-Maximum − ε), innerhalb der Gruppe neuestes zuerst. Echte Relevanzunterschiede (fehlendes Wort, Tippfehler, Titel- vs. Inhaltstreffer) liegen deutlich über ε und bleiben maßgeblich. **Nach dem Update einmal `venne-search:setup` ausführen** (oder irgendein Indexier-Vorgang), damit die neue Ranking-Rule in den bestehenden Index geschrieben wird.
